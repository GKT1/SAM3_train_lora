...
def _prune_messages_for_next_round(
    ...
    if part2 and len(previously_used) > 0:
        ...
        # Reconstruct message with all images
        content_list = []
        ...
        content_list.append({
            "type": "text",
            "text": f"The above images are the raw input images. The initial user input query is: '{initial_text_prompt}'."
            + " "
            + warning_text,
        })
        
        # Replace the second message entirely
        part1[1] = {
            "role": "user",
...
def agent_inference(
    ...
    """
    Given a text prompt and image(s), this tool will perform all aspects of agentic problem solving,
    while saving sam3 and MLLM outputs to their respective directories.

    Args:
        img_paths: List of paths to the input images (or single string)
    ...
    # init variables
    # Map image_idx to the latest output JSON for that image
    MASK_RESULTS_BY_IMAGE = {} 
    LATEST_IMAGE_IDX = 0
    LATEST_SAM3_TEXT_PROMPT = ""
    USED_TEXT_PROMPTS = (
        set()
    ...
    token_usage = {"prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0} # Track token usage

    # debug setup
    debug_folder_path = None
    debug_jsonl_path = None
    if debug:
        base_name = img_paths[0].rsplit('/', 1)[-1].rsplit('.', 1)[0]
        if len(img_paths) > 1:
            base_name += "_multi"
        debug_folder_path = os.path.join(debug_save_dir, base_name)
    ...
    for i, path in enumerate(img_paths):
        initial_content.append({"type": "text", "text": f"Image {i}: {os.path.basename(path)}"})
        initial_content.append({"type": "image", "image": path})
    ...
    # Force the display maximum to 5, but allow the loop to run up to 7 times.
    display_max_generations = 5
    max_generations = 7

    def send_request_with_retry(msgs, require_tool=True):
        max_retries = 4
        resp = None
        for attempt in range(max_retries):
            # Try passing token_tracker if supported
            try:
                resp = send_generate_request(msgs, token_tracker=token_usage)
            except TypeError:
                # Fallback if send_generate_request doesn't support token_tracker
                resp = send_generate_request(msgs)
            
            # Basic validity check: response should not be None or empty
            if resp:
                # If we require a tool tag, check for it
                if require_tool and "<tool>" not in resp:
                    print(f"⚠️ Warning: Response missing <tool> tag (Attempt {attempt+1}/{max_retries}). Retrying...")
                else:
                    return resp
            else:
                print(f"⚠️ Warning: Empty or None response (Attempt {attempt+1}/{max_retries}). Retrying...")
    ...
    # Add turn tracking info to the last user message if it's the start
    if messages[-1]["role"] == "user":
        display_turn = min(generation_count + 1, display_max_generations)
        turn_info = f"\n\n[Turn {display_turn} of {display_max_generations}]"
        
        # Add a special warning on the final turn
        if generation_count + 1 >= display_max_generations:
            turn_info += "\n⚠️ CRITICAL: Time is up! This is your FINAL turn. The only tools that will work now are select_masks_and_return or report_no_mask. You MUST call one of these now to provide your final answer, even if you have to randomly guess the answer."
    ...
    generated_text = send_request_with_retry(messages, require_tool=True)
    print(f"\n>>> MLLM Response [start]\n{generated_text}\n<<< MLLM Response [end]\n")
    while generated_text is not None:
        save_debug_messages(messages, debug, debug_folder_path, debug_jsonl_path)
        
        # Check if we have a valid tool tag before proceeding
        if "<tool>" not in generated_text:
             raise ValueError(f"Generated text does not contain <tool> tag after retries: {generated_text}")

        # --- Detect multiple tool calls or premature thought/action ---
        num_tools = generated_text.count("<tool>")
        if num_tools > 1:
            print(f"⚠️ Warning: MLLM provided {num_tools} tool calls in one turn. Enforcing protocol.")
            error_msg = f"Error: You provided {num_tools} tool calls in one turn. You MUST provide exactly ONE tool call per turn and wait for the system response. Do not hallucinate or predict mask counts before they are provided. Please repeat your thought process and provide ONLY the first intended tool call."
        ...
        if tool_call["name"] == "segment_phrase":
            ...
            if generation_count + 1 >= display_max_generations:
                print("❌ Intercepting segment_phrase tool call because time is up.")
                error_msg = f"Error: You have reached the maximum turn limit. The tool 'segment_phrase' is no longer available. You MUST call either select_masks_and_return or report_no_mask immediately with your final answer (random guess if necessary)."
                messages.append({"role": "assistant", "content": [{"type": "text", "text": generated_text}]})
                messages.append({"role": "user", "content": [{"type": "text", "text": error_msg}]})
                generation_count += 1
                generated_text = send_request_with_retry(messages, require_tool=True)
            ...
            # Extract image_idx if provided
            image_idx = tool_call["parameters"].get("image_idx", 0)
            if image_idx >= len(img_paths):
                print(f"❌ Invalid image_idx {image_idx}. Defaulting to 0.")
                image_idx = 0
            current_img_path = img_paths[image_idx]
            ...
            if False: # current_text_prompt in USED_TEXT_PROMPTS:
                print(
                ...
                path_to_output = call_sam_service(
                    image_path=current_img_path,
                    text_prompt=current_text_prompt,
                    output_folder_path=sam_output_dir,
                )
                MASK_RESULTS_BY_IMAGE[image_idx] = path_to_output
                sam3_outputs = json.load(open(path_to_output, "r"))
                ...
                if num_masks == 0:
                    ...
                    sam3_output_text_message = rf"The segment_phrase tool generated {num_masks} available masks on image {image_idx}. All {num_masks} available masks are rendered in this image below, now you must analyze the {num_masks} available mask(s) carefully, compare them against the raw input image and the original user query, and determine your next action. Please be reminded that the original user query was '{initial_text_prompt}'."
                    messages.append(
                        {
                            "role": "user",
                            "content": [
            ...
            print("🔍 Calling execute_code tool...")
            
            if generation_count + 1 >= display_max_generations:
                print("❌ Intercepting execute_code tool call because time is up.")
                error_msg = f"Error: You have reached the maximum turn limit. The tool 'execute_code' is no longer available. You MUST call either select_masks_and_return or report_no_mask immediately with your final answer (random guess if necessary)."
                messages.append({"role": "assistant", "content": [{"type": "text", "text": generated_text}]})
                messages.append({"role": "user", "content": [{"type": "text", "text": error_msg}]})
                generation_count += 1
                generated_text = send_request_with_retry(messages, require_tool=True)
            ...
            try:
                ...
                exec_globals.update({
                    "MASK_RESULTS_BY_IMAGE": MASK_RESULTS_BY_IMAGE,
                    "LATEST_IMAGE_IDX": LATEST_IMAGE_IDX,
                    "IMG_PATHS": img_paths
                })
                exec(code, exec_globals)
            except Exception as e:
                print(f"Error executing code: {e}")
                # We also capture it in stderr via print if not caught, but here we print to original stdout for log
            ...
            sam3_output_text_message = f"Code Execution Result:\nOutput:\n{output}\n"
            if error:
                sam3_output_text_message += f"Error:\n{error}\n"
            ...
            if generation_count + 1 >= display_max_generations:
                print("❌ Intercepting examine_each_mask tool call because time is up.")
                error_msg = f"Error: You have reached the maximum turn limit. The tool 'examine_each_mask' is no longer available. You MUST call either select_masks_and_return or report_no_mask immediately with your final answer (random guess if necessary)."
                messages.append({"role": "assistant", "content": [{"type": "text", "text": generated_text}]})
                messages.append({"role": "user", "content": [{"type": "text", "text": error_msg}]})
                generation_count += 1
                generated_text = send_request_with_retry(messages, require_tool=True)
            ...
            # Use current image index if provided, otherwise default to LATEST_IMAGE_IDX
            image_idx = tool_call["parameters"].get("image_idx", LATEST_IMAGE_IDX)
            if image_idx not in MASK_RESULTS_BY_IMAGE:
                raise ValueError(f"No masks available for image {image_idx}. Call segment_phrase first.")
            
            path_to_latest_json = MASK_RESULTS_BY_IMAGE[image_idx]

            # Make sure that the last message is a image
            if messages[-1]["role"] == "user" and isinstance(messages[-1]["content"], list) and len(messages[-1]["content"]) > 1 and messages[-1]["content"][1]["type"] == "image":
                ...
                simplified_message = {
                    ...
                    "content": [
                        {
                            "type": "text",
                            "text": f"The segment_phrase tool generated several masks on image {image_idx}. Now you must analyze the mask(s) carefully, compare them against the raw input image and the original user query, and determine your next action.",
                        }
                    ],
                }
                messages.append(simplified_message)

            current_outputs = json.load(open(path_to_latest_json, "r"))
            ...
            for i in range(num_masks):
                ...
                iterative_checking_messages = [
                    {"role": "system", "content": iterative_checking_system_prompt},
                    {
                        ...
                        "content": [
                            ...
                            {"type": "image", "image": current_outputs["original_image_path"]}, # Use the image associated with current outputs
                            {
                                "type": "text",
                ...
                checking_generated_text = None
                try:
                    checking_generated_text = send_generate_request(
                        iterative_checking_messages, token_tracker=token_usage
                    )
                except TypeError:
            ...
            MASK_RESULTS_BY_IMAGE[image_idx] = new_path
            json.dump(updated_outputs, open(new_path, "w"), indent=4)

        elif tool_call["name"] == "select_masks_and_return":
            print("🔍 Calling select_masks_and_return tool...")
            
            final_masks_by_image = tool_call["parameters"].get("final_answer_masks", {})
            textual_answer = tool_call["parameters"].get("answer", None)
            
            # Support both old list format and new dictionary format for robustness
            if isinstance(final_masks_by_image, list):
                final_masks_by_image = {str(LATEST_IMAGE_IDX): final_masks_by_image}
            
            all_final_boxes = []
            all_final_scores = []
            all_final_masks = []
            ref_image_path = img_paths[0]
            ref_h, ref_w = 0, 0

            for img_idx_str, masks_to_keep in final_masks_by_image.items():
                img_idx = int(img_idx_str)
                if img_idx not in MASK_RESULTS_BY_IMAGE:
                    print(f"⚠️ Warning: No masks found for image {img_idx}. Skipping.")
                    continue
                
                current_outputs = json.load(open(MASK_RESULTS_BY_IMAGE[img_idx], "r"))
                ref_image_path = current_outputs["original_image_path"]
                ref_h, ref_w = current_outputs["orig_img_h"], current_outputs["orig_img_w"]
                ...
                for i in valid_masks:
                    all_final_boxes.append(current_outputs["pred_boxes"][i-1])
                    all_final_scores.append(current_outputs["pred_scores"][i-1])
                    all_final_masks.append(current_outputs["pred_masks"][i-1])

            final_outputs = {
                "original_image_path": ref_image_path,
                "orig_img_h": ref_h,
                "orig_img_w": ref_w,
                "pred_boxes": all_final_boxes,
                "pred_scores": all_final_scores,
                "pred_masks": all_final_masks,
            ...
            textual_answer = tool_call["parameters"].get("answer", None)
            # Default to first image properties for reporting if no masks
            ref_img_path = img_paths[0]
            height, width = cv2.imread(ref_img_path).shape[:2]
            final_outputs = {
                "original_image_path": ref_img_path,
                "orig_img_h": height,
                "orig_img_w": width,
                "pred_boxes": [],
                "pred_scores": [],
            ...
            rendered_final_output = Image.open(ref_img_path)
            messages.append(
                {
                    "role": "assistant",
                    "content": [{"type": "text", "text": generated_text}],
        ...
        #     initial_text_prompt,
        # )
        # make sure we don't exceed allowed images (inputs + 1 mask output)
        # assert count_images(messages) <= len(img_paths) + 1
        generation_count += 1
        ...
        # Add turn tracking info to the last user message for the next round
        if messages[-1]["role"] == "user":
            display_turn = min(generation_count + 1, display_max_generations)
            turn_info = f"\n\n[Turn {display_turn} of {display_max_generations}]"
    ...
    print("⚠️ Agent exceeded maximum loop limit without calling select_masks_and_return or report_no_mask.")
    print("⚠️ Defaulting to an empty report_no_mask answer to prevent crashing and ensure json is returned.")
    
    error_save_path = os.path.join(
        error_save_dir,
        f"error_history.json",
    )
    with open(error_save_path, "w") as f:
        json.dump(messages, f, indent=4)
    print("Saved messages history that caused error to:", error_save_path)
    
    # Safely fallback to an empty response instead of crashing
    ref_img_path = img_paths[0]
    height, width = cv2.imread(ref_img_path).shape[:2]
    final_outputs = {
        "original_image_path": ref_img_path,
        ...
        "pred_scores": [],
        "pred_masks": [],
        "answer": "Failed to determine an answer within turn limits. Random guess required.",
        "token_usage": token_usage,
    }
    rendered_final_output = Image.open(ref_img_path)
    return messages, final_outputs, rendered_final_output
