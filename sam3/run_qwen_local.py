...
# LLM Configuration for Qwen 3 Local
LLM_PROVIDER = "local"
LLM_MODEL = "Qwen/Qwen3-VL-2B-Instruct"
...
def run_qwen_local_test():
    base_dir = os.path.dirname(os.path.abspath(__file__))
    samples_dir = os.path.join(base_dir, "assets/common_samples")
    output_base_dir = os.path.join(base_dir, "qwen_local_output")
    ...
    # API Key for local vLLM should match the server configuration ("EMPTY")
    # We ignore the environment variable here to prevent accidental usage of real OpenAI keys
    api_key = "EMPTY" 
